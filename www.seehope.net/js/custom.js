$(document).ready(function() {
	...
	*/
	...
	/**
	 * 如果首页手机端导航栏有二级菜单的话，显示箭头在一级菜单后提示用户
	 */
	$('.dl-menu>li').each(function() {
		if($(this).children('ul').html() != null) {
			$(this).children('a').append('<span class="fa fa-chevron-right" type="icon-nav"></span>');
		}
	});
	...
	$('.more_link').click(function() {
		...
		layer.open({
			type: 1,
			title: ['请关注我们的微信公众号以获取更多的内容'], //不显示标题栏
			...
			content: '<div style="text-align: center;padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;"><img src="img/seehopeITQRCode344.jpg"/></div>',
	...
	if($('.countdown').length) {
		$('.countdown').downCount({
			date: '08/10/2017 11:00:00', //距离倒计时开始的时间
			offset: +8 //设置时区
		});
	};
